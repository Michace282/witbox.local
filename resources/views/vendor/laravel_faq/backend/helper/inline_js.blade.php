<script>
    ...
    window['faq_grid_columns'] = [
        ...
        },
        {
            ...
            searchable: false,
            sortable: false,
            data: 'action', name: 'action', 'title': 'Title',
            mRender: function (data, type, full) {
                return '' +
                    ...
                    '       <i class="fa fa-trash"></i><span class="ml-2">Remove</span>' +
                    '   </a>'
                '  </div>' +
                '</div>';
    ...
    var create_faq_constraints = {
        title: {
            presence: {message: '<strong>The title of the form is required.</strong>'}
        },
    };
    ...
    function get_edit_faq_form(item_id) {
        ...
        $('#edit_faq').append(generate_loader_html('Please wait...'));
        $.ajax({
            type: "POST",
    ...
    function change_status_faq(input) {
        ...
        yesNoAlert('Change status', 'Are you sure you want to change the status of the tag?', 'warning', 'Crush Change status', 'Cancellation', set_faq_is_active, parameters, remove_checked, parameters);
    }
    ...
    function datatable_load_fun(filter_lang_id,filter_title,filter_is_active) {
        ...
        (
            ...
            '           <option value="0" '+('0' === filter_is_active ? 'selected="selected"' : '')+'>Inactive</option>' +
            '           <option value="1" '+('1' === filter_is_active ? 'selected="selected"' : '')+'>Active</option>' +
            '       </select>' +
            '   </td>' +
            html_td +
